...
import { Cart, CartItem, CartStore } from "@/type/cart-checkout-type";
...
const getCart = async ({
    ...
    const response = await api.get(
        `/cart?limit=${LIMIT_PAGE}&offset=${pageParam}&sort=${sort}`
    );
...
export const useCartQuery = () => {
    const query = useInfiniteQuery({
        ...
        getNextPageParam: (lastPage, allPages) =>
            lastPage?.cart?.stores?.length < LIMIT_PAGE
                ? null
                : allPages.length + 1,
        initialPageParam: 0,
        staleTime: 1000 * 60 * 5,
    ...
    const stores =
        query.data?.pages.flatMap((page) => page?.cart?.stores || []) ??
        ([] as (CartStore & {
            toko: Toko;
            items: (CartItem & {
                product: Product;
...
export const useCartMutations = () => {
    ...
    const increaseQuantity = useMutation<void, AxiosError<{ message?: string }>, string, MutationContext>({
    ...
    const decreaseQuantity = useMutation<void, AxiosError<{ message?: string }>, string, MutationContext>({
        mutationFn: async (cartStoreItemID: string) => {
...
export const useHandleCart = () => {
    ...
    const { increaseQuantity, decreaseQuantity, createCart, deleteItem } =
        useCartMutations();
